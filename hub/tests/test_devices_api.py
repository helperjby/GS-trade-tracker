"""hub 기기 등록·기기 토큰·공개/직접 구분·속도제한 — HUB-PROTOCOL §0·§3-0·§3-1·§3-6."""
...
from helpers import (AUTH, PROXY, app_db, body, device_auth, get, make_cfg, obs, post, register, row,
                     start_client)
...
def test_register_happy_path_no_bearer_and_token_hashed(tmp_path):
    async def scn():
        ...
        try:
            ...
            # label 생략·null 은 빈 문자열, 직접 접속의 created_ip 는 소켓 peer
            st3, data3 = await register(client, label=None)
            assert st3 == 200
...
def test_register_bad_invite_validation_full_and_closed(tmp_path):
    async def scn():
        ...
        try:
            ...
            st, data = await register(client, invite=123)
            assert (st, data["field"]) == (400, "invite_code")
            st, data = await register(client, invite="")
            assert (st, data["field"]) == (400, "invite_code")
...
def test_register_rate_limited_per_ip_uses_last_xff_entry(tmp_path):
    async def scn():
        ...
        try:
            ...
            assert app_db(app).count_active_devices() == 4
        finally:
            await client.close()
    _run(scn())
...
def test_device_token_mismatch_403_writes_nothing(tmp_path):
    async def scn():
        app, client = await start_client(make_cfg(tmp_path))
        try:
            ...
            # 400 검증이 403 보다 먼저 — 형식이 틀린 본문은 device_mismatch 로 새지 않는다
            ...
            (d,) = app_db(app).list_devices()
            assert d["upload_count"] == 0 and d["last_seen_ts"] is None
        finally:
            await client.close()
    _run(scn())
...
def test_auth_failure_storm_429_only_for_public_ip(tmp_path):
    async def scn():
        ...
        try:
            ...
            st, data = await post(client, body(obs()), headers=wrong)
            assert (st, data["error"]) == (429, "rate_limited")
            # 같은 공개 IP 는 올바른 기기 토큰이라도 창이 끝날 때까지 429
            st, data = await post(client, body(obs(), device_id=reg["device_id"]), headers={**device_auth(reg["token"]), **PROXY})
            assert st == 429
            # 다른 공개 IP 는 정상
            other = {**device_auth(reg["token"]), **PROXY, "X-Forwarded-For": "198.51.100.9"}
            st, data = await post(client, body(obs(obs_id="ok1"), device_id=reg["device_id"]), headers=other)
            assert st == 200
